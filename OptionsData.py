...
Y = options_data['T'].values
Z = options_data['implied_volatility'].values
print(options_data)

strike_grid = np.linspace(X.min(), X.max(), 100)
maturity_grid = np.linspace(Y.min(), Y.max(), 100)
X_grid, Y_grid = np.meshgrid(strike_grid, maturity_grid)
spline = SmoothBivariateSpline(X, Y, Z, s=1)  #
Z_smooth = spline.ev(X_grid.ravel(), Y_grid.ravel()).reshape(X_grid.shape)

fig = plt.figure(figsize=(12, 8))
ax = fig.add_subplot(111, projection='3d')

surf = ax.plot_surface(X_grid, Y_grid, Z_smooth, cmap='viridis', edgecolor='none')
ax.set_xlabel('Strike Price')
ax.set_ylabel('Time to Maturity (Years)')
ax.set_zlabel('Implied Volatility')
ax.set_title(f'Implied Volatility Surface for {ticker} Options')
